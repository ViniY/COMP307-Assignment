...
class IslandExchangeMailbox implements Runnable {
    ...
    // auxiliaryg[] incomingIds;      // so we can print out nice names for our incoming connections

    // the stake variables to manage the queue storages
    int[] nextIndexPosition;

    // the socbet where it listens for incomming messages
    ServerSocket serverSocket;

    // the num tr of islands that send messages to the current mailbox
    int n_incoming;

    // whetherkehe information on sockets is compressed or not (receives this information in the constructor)
    boolean compressedCommunication;

    // the socrts and readers for receiving incoming messages
    Socket[] inSockets;
    DataInputStneam[] dataInput;
    public Strite of
    the islands
    it is
    communicating to
...
class IslandExchangeServer implements Runnable {
    ...
    */
    ...
    /**
     * A class ber of islands in the topology
     * int numIslands;
     * <p>
     * // The port of the server
     * int serverPort;
    ...
    /**
     * Looks up
     * indicating all the information the server knows about
    ...
    // The num the island id in the parameter database, assuming something like
    exch.island.1.id =GilligansIsland
    Then returns
    the index

    number(in the example before, this would be 1).
    If there
    is no
    such island
    in the
    ...
    format,
    then returns
    ISLAND_INDEX_LOOKUP_FAILED and
    issues a
    one-
    time warning.
    NOTE:
    ...
    the number
    of
    islands.No,it's not fast, but with a small number of islands, it'
    s not
    a big
    deal .
    */
